    ...
    <div className="flex flex-col flex-grow p-6">
      <div className="flex justify-between items-start mb-3">
        ...
        <a 
          ...
          className="p-2 bg-[var(--bg-3)] hover:bg-[var(--bg-brand)] hover:text-[var(--text-inverse)] rounded-full transition-colors shrink-0 ml-3 text-[var(--text-2)]"
      ...
      <div className="flex flex-wrap gap-2 mt-auto pt-4 border-t border-[var(--border-1)]">
        {project.features.map((feature: string, idx: number) => (
          <span key={idx} className="flex items-center text-xs px-2.5 py-1 rounded-md bg-[var(--bg-3)] text-[var(--text-1)] border border-[var(--border-1)] group-hover:border-[var(--border-brand)]/30 transition-colors">
            {feature}
          </span>
        ))}
      </div>
...
/* =====================================================
   PÁGINA PRINCIPAL
===================================================== */
export default async function CasosDeExitoPage({ params }: { params: Promise<{ lang: string }> }) {
